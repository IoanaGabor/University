...
void ConsoleUI::printEvents(DynamicVector<Event> someEvents) {
    for (int i = 0; i < someEvents.getSize(); i++) {
        std::cout << someEvents[i].toString() << '\n';
    }
}
...
void ConsoleUI::handleUserDisplayEventsMenuCommand() {
    ...
    int currentIndex = 0;
    std::string commandAsLine;
    DynamicVector<Event> selectedEvents;
    try {
    ...
    while (true) {
        ...
        if (std::regex_search(commandAsLine, std::regex("^next$"))) {
            currentIndex++;
            currentIndex %= selectedEvents.getSize();
