...
void TestRepository::testAddAndListElementsRepository() {
    Repository<Event> repository = Repository<Event>();
    ...
    assert(repository.getAllEntities().getSize() == 4);
    try{
        repository.add(Event("dfas", "fdas", 3, "/2022/10/23/10:24","https://stackoverflow.com/questions/3385229"));
        assert(false);
...
void TestRepository::testRemoveRepository() {
    ...
    assert(repository.getAllEntities().getSize() == 3);
    try{
        repository.remove(Event("dfasdsaaa", "fdas", 3, "/2022/10/23/10:24","https://stackoverflow.com/questions/3385229").getId());
        assert(false);
    }catch (RepositoryException &repositoryException){
        assert(true);
    }
...
void TestRepository::testUpdateRepository() {
    ...
    assert(repository.getAllEntities().getSize() == 4);
    try{
        repository.update(Event("dfasdsaaa", "fdas", 3, "/2022/10/23/10:24","https://stackoverflow.com/questions/3385229"));
        assert(false);
    }catch (RepositoryException &repositoryException){
        assert(true);
    }
