...
public:
    ...
    DynamicVector<Event> listEventsAdministrator();

    void addEventUser(const std::string &id);

    void removeEventUser(const std::string &title, const std::string &dateAndTime);

    DynamicVector<Event> selectAllEventsByMonth(const std::string& month);

    DynamicVector<Event> listEventsUser();

    DynamicVector<Event> getAllEventsSortedByDate();
};
