...
#include "repository/FakeUserEventList.h"
...
void TestController::testAddEventUser_NotPresentInAdmin_ThrowsException() {
    FakeRepository<Event> *adminRepository = new FakeRepository<Event>(false);
    FakeUserEventList *userRepository = new FakeUserEventList("", true);
    EventsController *eventsController = new EventsController(*adminRepository, *userRepository);

    try {
        eventsController->addEventUser("sdf");
        assert(false);
    } catch (EventsControllerAdminException &eventsControllerException) {
        assert(true);
    }

    delete eventsController;
    delete adminRepository;
    delete userRepository;
}


void TestController::testAddEventUser_AlreadyPresentInUser_ThrowsException() {
    FakeRepository<Event> *adminRepository = new FakeRepository<Event>(true);
    FakeUserEventList *userRepository = new FakeUserEventList("", true);
    EventsController *eventsController = new EventsController(*adminRepository, *userRepository);
    ...
    try {
        eventsController->addEventUser("sdf");
        assert(false);
    } catch (EventsControllerUserException &eventsControllerException) {
        assert(true);
    }
...
void TestController::testAddEventUser_NewEvent_HappyFlow() {
    FakeRepository<Event> *adminRepository = new FakeRepository<Event>(true);
    Event eventToBeAddedToAdmin = Event("dfas", "fdas", 3, "/2022/10/23/10:24",
                                        "https://stackoverflow.com/questions/3385229");
    adminRepository->add(eventToBeAddedToAdmin);
    FakeUserEventList *userRepository = new FakeUserEventList("", false);
    EventsController *eventsController = new EventsController(*adminRepository, *userRepository);


    eventsController->addEventUser(
            Event::getIdFromTitleAndDate(eventToBeAddedToAdmin.getTitle(), eventToBeAddedToAdmin.getDateAndTime()));
...
void TestController::testsWithFakeRepository() {
    TestController::testAddEventUser_NotPresentInAdmin_ThrowsException();
    TestController::testAddEventUser_NewEvent_HappyFlow();
    TestController::testAddEventUser_AlreadyPresentInUser_ThrowsException();
}
