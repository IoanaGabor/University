...
class ConsoleUI:
    ...
    def __print_menu():
        ...
        print("11. Display all edges")
        print("12. Generate random graph")
        print("13. Read graph with normal format ")
        print("14. Write graph with normal format")
        print("15. Read graph with altered format")
        print("16. Write graph with altered format")
        print("17. Check if a vertex exists")
        print("18. Read activity graph")
        print("19. Solve scheduling problem")
        print("exit")

    @staticmethod
    def __read_a_vertex():
        vertex = input("Enter an activity:\n")
        duration = int(input("Enter the duration of the activity\n"))
        return vertex, duration

    @staticmethod
    def __read_an_edge_without_cost():
        print("The first vertex\n")
        x = ConsoleUI.__read_a_vertex()
        print("The second vertex\n")
        y = ConsoleUI.__read_a_vertex()
        return x, y
    ...
    def __handle_reading_activity_graph(self):
        filename = input("Enter the filename")
        try:
            self.__graph = SchedulingProblem.read_activities_from_file(filename)
        except Exception as exc:
            print(exc)

    def __handle_activities(self):
        try:
            duration, earliest, latest, critical = SchedulingProblem.solve(self.__graph)
            print("The total duration is {0}".format(duration))
            for vertex in earliest.keys():
                print("vertex {0} has earliest time {1} and latest time {2}"
                      .format(vertex, earliest[vertex], latest[vertex]))
            print("Critical vertices {0}".format(critical))
        except Exception as exc:
            print(exc)
    ...
    def run_console_menu(self):
        while True:
            ...
            if command == "1":
                ...
                self.__handle_reading_activity_graph()
            elif command == "19":
                self.__handle_activities()
            elif command == "exit":
                break
            else:
